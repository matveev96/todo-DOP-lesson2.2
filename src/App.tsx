...
function App() {
    ...
    const [todoes, setTodoes] = useState<TodoesType[]>([])

    useEffect(() => {
        setTodoes(todoFromServer.map(el => ({...el, todolistId: v1()})))
    }, [])

    function removeTask(id: string, todolistId: string) {
        setTodoes(prevState => prevState.map(el =>
    ...
    function changeFilter(payload:{filter: FilterValuesType, todolistId: string}) {
        ...
        setTodoes(prevState => prevState.map(el => el.todolistId === todolistId ? {...el, filter} : el))
    }

    function removeTodolist(id: string) {
        setTodoes(prevState => prevState.filter(el => el.todolistId != id))
    }
